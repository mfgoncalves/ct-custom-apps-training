...
const renderApp = (options: any = {}, includeManagePermissions = true) => {
  const route = options.route || `/my-project/${entryPointUriPath}/channels/${id}`;
...
const fetchChannelDetailsQueryHandler = graphql.query('FetchChannelDetails', (_req, res, ctx) => {
  return res(
    ctx.data({
...
const updateChannelDetailsHandler = graphql.mutation('UpdateChannelDetails', (_req, res, ctx) => {
  return res(
    ctx.data({
...
describe('rendering', () => {
  it('should render channel details', async () => {
    ...
    const keyInput = await screen.findByLabelText<HTMLInputElement>(/channel key/i);
    expect(keyInput.value).toBe(key);

    const nameInput = screen.getByRole<HTMLInputElement>('textbox', { name: /en/i });
  ...
  describe('when user has no manage permission', () => {
    it('should render the form as read-only and keep the "save" button "disabled"', async () => {
      useMockServerHandlers(fetchChannelDetailsQueryHandler, updateChannelDetailsHandler);
      renderApp({}, false);
...
describe('notifications', () => {
  it('should render a success notification after an update', async () => {
    ...
    within(notification).getByText(/Channel {channelName} updated/i);
  });
  it('should render an error notification if fetching channel details resulted in an error', async () => {
    ...
    renderApp();
    await screen.findByText(/please check your connection, the provided channel ID and try again/i);
  });
